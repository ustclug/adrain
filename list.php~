...
<br/>
	  <table class="table">
	   ...
	   <?php
			...
			$query = db::query("SELECT cid, dateofad, univname, funding, label, languagescore, subject, noteforchoose, schoolname, subsubject, noteforad FROM `case` WHERE `subject`='%s' ORDER BY dateofad", addslashes($_GET['y']));
