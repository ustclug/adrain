...
<br/>
	  <table class="table">
	   ...
	   <?php
			...
			$query = db::query("SELECT cid, dateofad, univname, funding, gpa, label, languagescore, subject, noteforchoose, schoolname, subsubject, noteforad FROM `case` WHERE `subject`='%s' ORDER BY dateofad", addslashes($_GET['y']));
		...
		?>
		<tr<?php echo (strpos($data['label'], 'REJ')!==FALSE?" class=\"warning\"":"")?>><td><?php echo $data['dateofad']?></td><td><?php echo $data['subsubject']?></td><td><?php echo $data['univname']?></td><td><?php echo $data['schoolname']?></td><td><?php echo $labelhtml?></td><td>
